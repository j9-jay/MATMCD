...
Git 저장소는 `E:\연구\MATMCD`, 외부 자산은 `E:\연구\MATMCD_DATA`다. 예시 폴더를 빈 채로 미리 만들지 않는다. 모델·체크포인트·실험 결과 폴더는 현재 생성하지 않았다. 설치 환경, 다운로드, 실제 분석 산출물과 공식 코드가 요구하는 요약 저장 경로만 준비했다.
...
- CODE-03(요약 저장 부모 폴더 누락)은 [TASK_011](tasks/TASK_011_summary_output_directory.md)에서 해결했다. 공식 코드 변경 없이 필수 경로만 준비했고 WSL 쓰기 권한을 확인했다.
